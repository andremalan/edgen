...
if ( ! function_exists( 'create_page_layout' ) ) :
	...
	function create_page_layout($source = 'post') {
		...
		if (!is_array($layouts) || count($layouts) <= 0 ) {
			...
			//................................................................
			// This contains the main structure or outline of the design. The internal parts of the template are populated 
			// using TI (Template Inheritance) to include content blocks as needed.
...
if ( ! function_exists( 'theme_get_page' ) ) :
	...
	function theme_get_page($id, $title = false) {
		...
		// this function checks the global and individual page settings and returns true/false
		$remove_filter = wpautop_filter($id);
		
		// check if wpautop is already removed. if not, remove it because we want it off before we get the content
		...
		function content_type_specific($content, $id) {
			...
			switch ( get_post_type($id) ) {
				...
				case "static_block":
					...
					return '<div class="staticBlockWrapper ugc" '.$background.'><div class="staticContent">' . $content . '<div class="clear"></div></div></div>';
		...
		if ( is_array($ss) ) {
			...
			for ($n = 1; $n <= $cols; $n++) {
				...
				if ( is_array($slides) ) {
					...
					// ideahack : create var to store URLs of slider images
					$slideimages = array();
					...
					while ( have_posts() ) : the_post();
						//$slide_product_array[$slide_count]['media'] = wp_get_attachment_image_src( get_post_thumbnail_id());
					...
					$featured_media = wpsc_the_product_thumbnail(get_option('product_image_width'),get_option('product_image_height'),'','single');
					$featured_thumbnail = get_the_post_thumbnail(get_the_ID(), array(100,100));
					$featured_thumbnail = explode('"', $featured_thumbnail);
					$featured_thumbnail = $featured_thumbnail[5];
											
					$featured_link = post_permalink();
					$slide_product_array[$slide_count]['media'] = $featured_media;
					$slide_product_array[$slide_count]['thumbnail'] = $featured_thumbnail;		
							
					$slide_product_array[$slide_count]['link'] = $featured_link;
					$slide_product_array[$slide_count]['content'] = "
					...
					<a href='$featured_link'>
						<div id='featured_content'>
							...
							<div class='featured_content'>
								...
								<div id='donate'>
								<div style='width: 230px; float: left; margin-right: 150px;'>
  							  <div class='meter'>
            			  <span style='width: $progress%'></span>
            		  </div>
                  <div id='raised'> $ $raised raised out of $ $funding_needed</div>
                </div>
                <div style='margin-top: 20px; float: right;'>
								  [button style='white large']Click to Donate[/button]
                </div>
								</div>
							</div>
						</div>
					...
					foreach ($slides as $slide) {
						...
						//ideahack, overwrite slides here:
						if($slide['format'] == 'content'){
							$slide['link'] = $slide_product_array[$slide_count]['link']; 
							$slide['media']	= $slide_product_array[$slide_count]['media'];
							$slide['content'] = $slide_product_array[$slide_count]['content'];
						}
						 
						array_push($slideimages, $slide['media']);
					
						
						display_slide($slide, $w, $h);
							//$slideimages = $slide['media'];
					...
					// ideahack variables for displaying thumbnails
					//$thumbsourcevar = '<li><a href="#"><img src="http://lorempixum.com/g/100/100" width="100" height="100" /></a></li>';
					//$thumbvar = "function(idx, slide) {return '". $thumbsourcevar . "'";
			...
			<style>
				...
				#slide1 a {
					background-image: url(\'' . $slide_product_array[0]['thumbnail'] . '\');
					width: 100px;
					height: 100px;
					float: left;
				}
				...
				#slide2 a {
					background-image: url(\'' . $slide_product_array[1]['thumbnail'] . '\');
					width: 100px;
					height: 100px;
					float: left;
				...
				#slide3 a {
					background-image: url(\'' . $slide_product_array[2]['thumbnail'] . '\');
					width: 100px;
					height: 100px;
					float: left;
				...
				#slide4 a {
					background-image: url(\'' . $slide_product_array[3]['thumbnail'] . '\');
					width: 100px;
					height: 100px;
					float: left;
				...
				#slide5 a {
					background-image: url(\'' . $slide_product_array[4]['thumbnail'] . '\');
					width: 100px;
					height: 100px;
					float: left;
				...
				#slide6 a {
					background-image: url(\'' . $slide_product_array[5]['thumbnail'] . '\');
					width: 100px;
					height: 100px;
					float: left;
				...
				#slide7 a {
					background-image: url(\'' . $slide_product_array[6]['thumbnail'] . '\');
					width: 100px;
					height: 100px;
					float: left;
			...
			echo '<div id="'. $ssID .'_nav" class="slideShowPager"></div>'; // <img src="'$slideimages[0]'">
			...
			echo '<script type="text/javascript"> jQuery(document).ready(function($) { ';
				...
				foreach ($scripts as $key => $value) {
					...
					// ideahack echo 'pagerAnchorBuilder: function(id, slide) {return "<li><a href="#"><img src="' + slide.src + '" width="150" height="150" /></a></li>"};'
