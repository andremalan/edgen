...
function bp_groupblogs_fetch_group_feeds( $group_id = false ) {
	...
	/* Record found blog posts in activity streams */ // ideahack
	foreach ( (array) $items as $post_date => $post ) {
		$activity_action = sprintf( __( '<div id="twitter_icon"></div><div id="twitter_content" style="float:right;">%s</div>', 'bp-groups-externalblogs' ), '<a href="' . attribute_escape( $post['link'] ) . '">' . attribute_escape( $post['title'] ) . '</a>', '<a href="' . attribute_escape( $post['blogurl'] ) . '">' . attribute_escape( $post['blogname'] ) . '</a>', '<a href="' . bp_get_group_permalink( $group ) . '">' . attribute_escape( $group->name ) . '</a>' );
	
	// display excerpt only if not the widget on home page (ideahack)
	
//	if (is_front_page() ) :
	
	$activity_content = '<blockquote>' . strip_tags( bp_create_excerpt( $post['description'], 100 ) ) . '</blockquote>';
