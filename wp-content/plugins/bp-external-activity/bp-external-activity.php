...
function ih_external_activity_fetch_feeds( ) {
...
add_action( 'ih_external_activity_cron', 'ih_external_activity_fetch_feeds' );
//add_action( 'bp_before_activity_loop', 'ih_external_activity_fetch_feeds' );

/* Add a filter option to the filter select box on group activity pages */
function ih_external_activity_add_filters() {
...
add_action( 'bp_group_activity_filter_options', 'ih_external_activity_add_filters' );
add_action( 'bp_activity_filter_options', 'ih_external_activity_add_filters' );



/* Fetch group twitter posts after 30 mins expires and someone hits the group page */
function ih_external_activity_refetch() {
	global $bp;

	$last_refetch = groups_get_groupmeta( $bp->groups->current_group->id, 'ih_external_activity_lastupdate' );
	if ( strtotime( gmdate( "Y-m-d H:i:s" ) ) >= strtotime( '+30 minutes', strtotime( $last_refetch ) ) )
		add_action( 'wp_footer', '_ih_external_activity_refetch' );

	/* Refetch the latest group twitter posts via AJAX so we don't stall a page load. */
	//function _ih_external_activity_refetch() {
	//	global $bp; ?>
		...
		groups_update_groupmeta( $bp->groups->current_group->id, 'ih_external_activity_lastupdate', gmdate( "Y-m-d H:i:s" ) );
	//}
}
add_action( 'bp_before_activity_loop', 'ih_external_activity_refetch' );

/* Refresh via an AJAX post for the group */
function ih_external_activity_ajax_refresh() {
	ih_external_activity_fetch_feeds( $_POST['group_id'] );
}
add_action( 'wp_ajax_refetch_external_activity', 'ih_external_activity_ajax_refresh' );
