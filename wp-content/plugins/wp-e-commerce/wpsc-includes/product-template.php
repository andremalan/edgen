...
function ih_percent_raised($quantity, $quantity_bought, $price = 20){
// 
 	$total_needed = $price * $quantity;
 	$total_raised = $price * $quantity_bought;
 	$percent_raised = $total_raised / $total_needed;
// 
// 
	// return 'hello world';
...
function ih_quantity_display($the_product_id = 0, $in_cart = 1, $type = "product", $cart_name ="irrelevant", $cart_total = 0 ) {
 	$price = get_post_meta( $the_product_id, '_wpsc_price', true );
	//$quantity = get_product_meta( $the_product_id, 'stock',true );	
	//$quantity = wpsc_product_remaining_stock($the_product_id);
	$funding_raised = get_post_meta($the_product_id, 'Funding Received', true);
	$funding_needed = get_post_meta($the_product_id, 'Funding Needed', true);
	$funding_left = $funding_needed - $funding_raised;
	$quantity = $funding_left / $price;
	if( "product" == $type ) {
		$name = "wpsc_quantity_update";
	}
	else {
		$name = "quantity";
	}
	...
	if(0 < $quantity){
		if ( $in_cart > $quantity)
			$in_cart = $quantity;
		echo '$ <select id="wpsc_quantity_update_' . $the_product_id . '" name="' .$name. '">';
		...
		for ($i=1; $i <= $quantity; $i++){
			...
			if ( $i == $in_cart){
		 	...
		 	echo '<option ' . $selected. ' value="' . $i . '">'. $price * $i . '</option>';
		}
	
			echo "</select>";
	}
	elseif("product" == $type) {
		echo'<input type="text" id="wpsc_quantity_update_'. $the_product_id .'" name="wpsc_quantity_update" size="2" value="1" />';
		
	}
	
	elseif($cart_name != "Donation"){
		echo '<input type="text" name="quantity" size="2" value="' .  $in_cart . '" />';
	}
	
	else{

			
			$total_before_donation = $cart_total - $in_cart;
			
			
			$percentage = ceil(round((100 * $in_cart) / $total_before_donation)/5)*5;
			// var_dump($total_before_donation);
	//echo '$ <input type="text" id="wpsc_quantity_update_' . $the_product_id . '" name="' .$name. '" value="0">';
	...
	//echo "cart quantity = " . $in_cart . "    ";
	
	
	//echo "<input type='text' id='donation_widget_price_" . $the_product_id . "' name='donation_price' value='" . esc_attr( number_format( $price, 2 ) ) . "' size='6' />";
		//echo '$ <input type="text" name="update_donation" size="2" value="'. wpsc_cart_item_price(false) .  '" />';
		//echo '<input type="hidden" name="product_id" value="'. $the_product_id . '" />';	
	}
}
