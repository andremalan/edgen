...
function wpsc_donations( $args = null ) {
	...
	if ( $products != null ) {
		foreach ( $products as $product ) {
			...
			$output .= "<div class='donation_form'>";
			$output .= "<form class='product_form donation_widget'  name='donation_widget_" . $product['ID'] . "' method='post' action='' id='donation_widget_" . $product['ID'] . "'>";
			...
			$output .= "<label for='donation_widget_price_" . $product['ID'] . "'>" . __( 'Donation', 'wpsc' ) . ":</label> $currency_symbol<input type='text' id='donation_widget_price_" . $product['ID'] . "' name='donation_price' value='" . esc_attr( number_format( $price, 2 ) ) . "' size='6' />";
